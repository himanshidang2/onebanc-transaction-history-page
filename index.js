...
const fetchPromise = fetch("https://dev.onebanc.ai/assignment.asmx/GetTransactionHistory?userId=1&recipientId=2");
...
function custom_sort(a, b) {         //this function will sort the json data on behalf of StartDate

    return new Date(a.startDate).getTime() - new Date(b.startDate).getTime();
}

function MyDataSort(arr){            // basic function created to call the sorting function
...
function myData(data){            
    ...
    for(let  i = 0; i < arr.length ; i++){
        ...
        date += d + ' ' + monthNames[m] + ' ' + year;   //changing the format of date eg. "25-02-2021"
        ...
        if (i == 0 || arr[i].startDate.toString().substring(0, 10) != arr[i - 1].startDate.toString().substring(0,10))
        {
            inputs += '<p class="date">' + date + '</p>';
        }

        if (arr[i].direction === 1) {     //checking direction 1-sent or 2 - receieved
            if (arr[i].type === 1) {
        ...
        inputs += '<span class="left-pane-text">';
        inputs += '<span>' + '&#8377; ' + amount  + '</span>';  //adding amount to left div text
        // inputs += '<span>' + '&#8377; ' + amount  + '</span><p>' + description + '</p>';  //adding amount to left div text


        if (arr[i].status === 1) {      
            if (arr[i].direction === 1) {
                inputs += '<button class="btn">Cancel</button> </span >';
            }
            if (arr[i].direction === 2) {
                inputs += '<span class="buttons"><button class="btn-sm" > Pay</button >';
                inputs += '<button class="btn">Decline</button></span ></span>';
            }
        }
        if (arr[i].status === 2) {
            inputs += '<p> Transaction ID</p><p id="transId">' + id + '</p></span>';
        }

        inputs += '<span class="right-pane-text"><span><i class="fa fa-';
        ...
        inputs += '<i class="fa fa-chevron-right"></i></span></div >';
        ...
        newDiv.onclick = function(event){
            ...
            modal += '<img id = "cross" src ="images/close.png" alt = "" >'
 

             
             modal+='<h5> Transaction ID :'+arr[i].id+' '+"</h5>";
             modal+='<h5> Amount         :'+arr[i].amount+' '+"</h5>";
             modal+='<h5> Customer Email :'+arr[i].customer.vPay+' '+"</h5>";
             modal+='<h5> Customer ID    :'+arr[i].customer.vPayId+' '+"</h5>";
             modal+='<h5> Partner Email  :'+arr[i].partner.vPay+' '+"</h5>";
             modal+='<h5> Partner ID     :'+arr[i].partner.vPayId+' '+"</h5>";
             modal+='<h5> Description    :'+arr[i].description+' '+"</h5>";
